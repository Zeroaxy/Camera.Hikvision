...
namespace PreviewDemo
{
    public partial class Login : Form
    {
        ...
        private Int32[] m_lUserID = new Int32[] { -1, -1, -1, -1, -1, -1, -1, -1, -1 };      //用户登录标志位
        private bool m_bInitSDK = false;                                                     //初始化标志位
        public static Int32[] m_lRealHandle = new Int32[] { -1, -1, -1, -1, -1, -1, -1, -1, -1 };  //预览标志位
        ...
        public Login()
        {
            ...
            #region  数据记忆
            textBox1.Text = Settings.Default.port1;
            textBox2.Text = Settings.Default.port2;
            textBox3.Text = Settings.Default.port3;
            textBox4.Text = Settings.Default.port4;
            textBox5.Text = Settings.Default.port5;
            textBox6.Text = Settings.Default.port6;
            textBox7.Text = Settings.Default.port7;
            textBox8.Text = Settings.Default.port8;
            textBox9.Text = Settings.Default.port9;
            textBoxport.Text = Settings.Default.ip;
            textBoxUserName.Text = Settings.Default.name;
        ...
        private void DVR_Login_out(int num, string textBox_Text,Label labels)
        {
            if (textBox_Text != "")
            {
                if (m_lUserID[num] < 0)
                    labels.Text = DVR_Login(num, textBox_Text);
            }
            else
            {
                if (m_lUserID[num] >= 0)
                {
                    labels.Text = DVR_Logout(num);
                }
            }
        }
        ...
        {
            //设备IP地址或者域名
            byte[] byIP = System.Text.Encoding.Default.GetBytes(textBox_Text);
